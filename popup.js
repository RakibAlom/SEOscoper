...
/**
 * Direct element highlighter executed on the active tab via chrome.scripting.executeScript
 */
function highlightElementOnPage(type, index) {
  ...
  chrome.scripting.executeScript({
    ...
    if (typeof chrome !== 'undefined' && chrome.tabs?.sendMessage) {
      chrome.tabs.sendMessage(currentTabId, { action: 'HIGHLIGHT_ELEMENT', type, index });
    }
  });
}
...
function initTheme() {
  if (typeof chrome !== 'undefined' && chrome.storage?.local) {
    ...
    const saved = safeGetStorage('seoscoper_theme');
    if (saved) {
      setTheme(saved);
    } else {
      const prefersDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
  ...
  if (toggleBtn) {
    toggleBtn.addEventListener('click', () => {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage?.local) {
        ...
        safeSetStorage('seoscoper_theme', newTheme);
      }
    });
  }
}
...
function initTabNavigation() {
  ...
  tabButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage?.local) {
        ...
        safeSetStorage('seoscoper_last_tab', targetTab);
      }
    });
  });
  ...
  if (typeof chrome !== 'undefined' && chrome.storage?.local) {
    chrome.storage.local.get(['lastTab'], (result) => {
      if (result && result.lastTab && document.getElementById(`pane-${result.lastTab}`)) {
        switchTab(result.lastTab);
      }
    });
  } else {
    const savedTab = safeGetStorage('seoscoper_last_tab');
...
async function loadActiveTabAndAnalyze(forceRefresh = false) {
  ...
  // Check if running inside active Chrome extension context
  const isExtensionContext = typeof chrome !== 'undefined' && chrome.tabs?.query && chrome.runtime?.id;

  if (!isExtensionContext) {
    showRestrictedState('Chrome Extension context required. Please open SEOscoper from the Chrome toolbar while on any live webpage to audit.');
    return;
  }
  ...
  try {
    ...
    // Check for restricted chrome/internal pages
    if (isRestrictedUrl(currentTabUrl)) {
      showRestrictedState('SEOscoper cannot analyze internal browser pages (such as chrome://, Web Store, or PDF viewers). Please open any live webpage (e.g. easyprotools.com).');
      return;
    }

    // Direct live in-page DOM extraction via chrome.scripting.executeScript
    try {
      ...
      console.warn('[SEOscoper] Direct script execution error, attempting content script fallback:', scriptErr);
    }
    ...
    console.error('[SEOscoper] Tab query error:', err);
...
function fetchSEODataFromContentScript(tabId, forceRefresh = false) {
  if (typeof chrome === 'undefined' || !chrome.tabs?.sendMessage) {
    showRestrictedState('Chrome extension messaging API unavailable.');
    return;
  }

  chrome.tabs.sendMessage(tabId, { action: 'EXTRACT_SEO_DATA' }, (response) => {
    if (chrome.runtime.lastError || !response || !response.success) {
      ...
      chrome.scripting.executeScript({
        ...
        if (chrome.runtime.lastError) {
          console.warn('[SEOscoper] Injection error:', chrome.runtime.lastError.message);
          showRestrictedState('Cannot access this page. Make sure you have permission to run extensions here.');
          return;
        }

        // Retry sending message after script injection
        setTimeout(() => {
          chrome.tabs.sendMessage(tabId, { action: 'EXTRACT_SEO_DATA' }, (retryResponse) => {
